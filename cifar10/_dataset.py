...
class CIFAR(torch.utils.data.Dataset):
    ...
    def __init__(self, x, y, transform=None):
        ...
        y: numpy.array, float
        """
        self.x = x.transpose((0, 3, 1, 2))  # 32x32x3 => 3x32x32
...
def load(aug=False, batch_size=30):
    ...
    if aug:
        augmentation = torchvision.transforms.Compose([
            RandomErasing(num=3, size=5),
            GaussianNoise()
        ])
    else:
        augmentation = torchvision.transforms.Compose([])

    set_train = CIFAR(x_train, y_train, transform=augmentation)
    set_test = CIFAR(x_test, y_test)
